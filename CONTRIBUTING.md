...
- `src/lib/core/` — pure, browser-free logic (the trail reducer, jump planner,
  and trigger matcher). Unit-tested directly.
...
- `src/lib/ui/` — the breadcrumb overlay and shared settings controls.
...
naming conventions, and overlay contracts — keep `core` pure and don't let `ui`
